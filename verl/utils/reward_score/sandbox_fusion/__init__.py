...
def compute_score(
    ...
    Args:
        ...
        max_cases: If set, cap the number of test cases to this value. Useful during
                   validation to reduce wall-clock time (e.g. max_cases=5 cuts 37→5 cases,
                   ~7x speedup). Training uses all cases (max_cases=None).
    """
    solution = completion
    ...
    try:
        ...
        # Cap test cases for faster validation (no impact on training accuracy signal).
        if max_cases is not None and max_cases > 0:
            for key in ("inputs", "outputs", "assert_case"):
                if key in test_cases and isinstance(test_cases[key], list):
                    test_cases[key] = test_cases[key][:max_cases]
