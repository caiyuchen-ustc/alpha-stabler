...
def _process_single_case(
    ...
    fail_fast_event: Optional[threading.Event] = None,
) -> tuple[int, dict[str, Any]]:
    """Helper function to process a single test case."""
    # Fail-fast: skip this case immediately if a previous case already failed.
    if fail_fast_event is not None and fail_fast_event.is_set():
        return False, {"case_index": case_index, "status": "skipped_fail_fast"}
...
"""
    ...
    try:
        if concurrent_semaphore:
            with concurrent_semaphore:
                # Re-check after acquiring semaphore (may have waited a while).
                if fail_fast_event is not None and fail_fast_event.is_set():
                    return False, {"case_index": case_index, "status": "skipped_fail_fast"}
    ...
    # Fail-fast: signal remaining cases to skip if this one didn't pass.
    if fail_fast_event is not None and result_status is not True:
        fail_fast_event.set()
...
def check_correctness(
    ...
    # One event per generation: set when any case fails, so queued cases skip.
    fail_fast_event = threading.Event() if fail_fast else None

    if fail_fast:
        # Serial execution: stop on first failure (no point running remaining cases).
        # Much faster for wrong answers (typically fail on case 0 or 1).
        for i, stdin_data in enumerate(inputs):
            if fail_fast_event.is_set():
                results[i] = False
                metadata_list[i] = {"case_index": i, "status": "skipped_fail_fast",
                                     "input": str(inputs[i]), "expected_output": str(expected_outputs[i]) if expected_outputs[i] else None}
                continue
            result_status, metadata = _process_single_case(
                i, stdin_data, expected_outputs[i], sandbox_fusion_url,
                generation + "\n\n" + assert_cases[i],
                timeout, memory_limit_mb, language,
                concurrent_semaphore, fn_name, fail_fast_event,
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max(32, os.cpu_count() * 5)) as executor:
            future_to_index = {
                executor.submit(
                    ...
                    generation + "\n\n" + assert_cases[i],
                    timeout, memory_limit_mb, language,
                    concurrent_semaphore, fn_name, None,
                ): i
                for i, stdin_data in enumerate(inputs)
            }
