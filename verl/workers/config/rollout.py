...
class RolloutConfig(BaseConfig):
    ...
    # Random seed for the rollout engine (vLLM). Used e.g. to draw independent samples across
    # separate offline generation rounds.
    seed: int = 0
    ...
    # Off-policy distillation: when True, the vLLM rollout engine serves a frozen
    # teacher checkpoint (loaded from `teacher_model_path`) instead of the student
    # actor, and the per-step student->vLLM weight sync is skipped. The student still
    # trains via the reverse-KL-to-teacher advantage, i.e. SFT-style distillation on
    # teacher-generated rollouts.
    off_policy_rollout: bool = False
    teacher_model_path: Optional[str] = None

    # Offline distillation: when True, generate_sequences is replaced by a loader that reads
    # pre-generated teacher responses from `offline_teacher_data_path` (a parquet with a prompt
    # column and `offline_teacher_response_key`), tokenizes them, and returns them as the rollout
    # batch. No vLLM sampling happens. Combined with only_reverse_kl_advantages=true this trains
    # the student with reverse-KL to the teacher on the offline teacher rollouts.
    offline_teacher_rollout: bool = False
    offline_teacher_data_path: Optional[str] = None
    offline_teacher_response_key: str = "response"
