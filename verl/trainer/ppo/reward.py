...
def load_reward_manager(
    config: DictConfig, tokenizer: Any, num_examine: int, max_cases_override: Any = None, **reward_kwargs: Any
    ...
    if compute_score is None:
        ...
        if sandbox_url:
            ...
            _run_timeout = sandbox_config.get("run_timeout", 10)
            # max_cases: cap test cases per problem. None = all.
            # max_cases_override lets the caller (main_ppo) set different values for
            # training vs validation reward managers.
            _max_cases = max_cases_override if max_cases_override is not None else sandbox_config.get("max_cases", None)
