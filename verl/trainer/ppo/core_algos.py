...
@register_policy_loss("fused_future_adv")
def compute_policy_loss_fused_future_adv(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    """
    Full-future fused advantage correction + PPO + dual-clip + sequence filter

    Design:
        1) First compute all future_raw for the whole sequence:
           future_raw[:, t] = sum_{k=t+1}^T gamma^(k-t) * A_k^(m)

        2) After the loop is fully done, clip all positions together:
           future_used = clip(future_raw, -adaptive_bound, adaptive_bound)

        3) Then fuse once:
           adv_final = adv_masked + future_lambda * future_used

    This matches the requested behavior:
        - future_t_used is NOT computed inside the loop
        - clip happens only after all future_raw are completed
    """
    ...
    future_gate_eta = 0.05
    future_gate_power = 1.0  # fixed p = 2
    ...
    # ==================================================
    # 3) adaptive bound
    #    B_t = c * |A_t^(m)| * ( |A_t^(m)| / (|A_t^(m)| + eta) )^2
    # ==================================================
    abs_adv = adv_masked.abs()

    adaptive_bound = (
        abs_adv
        * (abs_adv / (abs_adv + future_gate_eta)).pow(future_gate_power)
    )

    adaptive_bound = torch.where(
        abs_adv > eps,
        adaptive_bound,
        torch.zeros_like(adaptive_bound),
    )
    
    # ==================================================
    # 4) compute all future_raw first
    #
    # future_raw[:, t] = sum_{k=t+1}^T gamma^(k-t) * A_k^(m)
    # ==================================================
    future_raw = torch.zeros_like(adv_masked)

    # suffix_sum will represent:
    #   A_{t+1}^(m) + gamma A_{t+2}^(m) + gamma^2 A_{t+3}^(m) + ...
    suffix_sum = torch.zeros(B, device=device, dtype=dtype)

    for t in reversed(range(T)):
        future_raw[:, t] = gamma * suffix_sum
        suffix_sum = adv_masked[:, t] + gamma * suffix_sum

    future_raw = future_raw * mask

    # ==================================================
    # 5) after all future_raw are computed, clip once together
    # ==================================================
    future_used = torch.clamp(
        future_raw,
        min=-adaptive_bound,
        max=adaptive_bound,
    )
    future_used = future_used * mask

    # ==================================================
    # 5.5) sign-consistency gate
    # only keep future reward if it has the same sign as current advantage
    # ==================================================
    same_sign_mask = (adv_masked * future_used) > 0
    same_sign_mask = same_sign_mask.to(dtype)
    
    future_used = future_used * same_sign_mask
    future_used = future_used * mask

    # ==================================================
    # 6) fuse after clipping
    #    adv_final = adv_masked + lambda * future_used
    # ==================================================
    adv_final = adv_masked + future_used
    adv_final = adv_final * mask

    # breakpoint()
    breakpoint()
    advantages = adv_final
    # ==================================================
    #  PPO ratio
    # ==================================================
    assert config is not None
