...
class FSDPSFTTrainer:
    def __init__(
        ...
        # Trainable token vector ("single" distillation mode): train only a small steer vector
        # added at selected decoder layers, with the base model frozen.
        self.use_trainable_vector = bool(self.config.model.get("enable_trainable_token_vector", False))
        if self.use_trainable_vector and self.lora:
            raise ValueError("enable_trainable_token_vector and LoRA cannot be used together in SFT.")
    ...
    def _build_model_optimizer(self):
        ...
        # Trainable token vector ("single" mode): install steer-vector hooks, freeze the base,
        # and keep only the vector parameters trainable. Reuses the same util as the RL worker.
        self._trainable_vector_ignored_states = None
        if self.use_trainable_vector:
            from verl.workers.fsdp_workers import (
                _get_trainable_vector_hook_modules,
                _is_trainable_vector_param_name,
                install_trainable_token_vector_hooks,
            )

            if self.config.model.strategy != "fsdp":
                raise ValueError(
                    "enable_trainable_token_vector=True in SFT requires model.strategy=fsdp "
                    "(FSDP1 with use_orig_params=True); fsdp2 is not supported for vector mode."
                )

            try:
                vector_dtype = next(self.model.parameters()).dtype
            ...
            installed_hooks = install_trainable_token_vector_hooks(
                ...
                dtype=vector_dtype,
                num_vectors=int(self.config.model.get("trainable_token_vector_num", 1)),
                sampling_method=str(self.config.model.get("trainable_token_vector_sampling_method", "hypersphere")),
                vector_scale=float(self.config.model.get("trainable_token_vector_scale", 0.1)),
                learnable_alpha=bool(self.config.model.get("trainable_token_vector_learnable_alpha", False)),
                alpha_init=float(self.config.model.get("trainable_token_vector_alpha_init", 0.0)),
                curriculum="none",
            )
            ...
            if len(vector_params) == 0:
                raise RuntimeError("enable_trainable_token_vector=True but no steer_vector params were found.")

            self.model._single_trainable_vector_force_all_tokens = bool(
                self.config.model.get("trainable_token_vector_force_all_tokens", True)
            )
            self._trainable_vector_ignored_states = _get_trainable_vector_hook_modules(self.model)

            if self.device_mesh.get_rank() == 0:
                print(
                    f"[SFT] Installed {len(installed_hooks)} trainable vector hook(s); "
                    f"training {len(vector_params)} vector param(s), base frozen."
                )
        ...
        if fsdp_strategy == "fsdp":
            fsdp_extra_kwargs = {"use_orig_params": False}
            if self.use_trainable_vector:
                # Vector params must stay as ordinary parameters (not flattened) so the optimizer
                # can update only them; the hook modules are excluded via ignored_states.
                fsdp_extra_kwargs["use_orig_params"] = True
                if self._trainable_vector_ignored_states:
                    fsdp_extra_kwargs["ignored_states"] = self._trainable_vector_ignored_states
    ...
    def _validate(self, global_step, tracking, rank):
        """Run the full validation loop and (on rank 0) log the mean val loss.

        Returns the metric dict on rank 0, else None.
        """
        val_losses = []
        ...
        if rank == 0:
            if len(val_losses) > 0:
                ...
                print(f"[validation] step={global_step} val/loss={metric['val/loss']:.6f}")
            else:
                print(f"[validation] step={global_step} skipped: empty val dataloader")
    ...
    def fit(self):
        ...
        # Validate the base model before any training (only when starting fresh, not resuming).
        val_before_train = self.config.trainer.get("val_before_train", True)
        if val_before_train and global_step == 0:
            log_with_rank(
                "Running validation on the base model before training...",
                logger=logger,
                rank=rank,
                log_only_rank_0=True,
            )
            base_metric = self._validate(global_step=0, tracking=tracking, rank=rank)
            if rank == 0 and base_metric is not None:
                last_valid_metric = base_metric
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                did_validate_this_step = False
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    metric = self._validate(global_step=global_step, tracking=tracking, rank=rank)
                    if rank == 0 and metric is not None:
                        last_valid_metric = metric
                    did_validate_this_step = True
            ...
            # End-of-epoch validation (skip if the final step of this epoch already validated).
            if not did_validate_this_step:
                log_with_rank(
                    f"Running end-of-epoch validation after epoch {epoch + 1}...",
                    logger=logger,
                    rank=rank,
                    log_only_rank_0=True,
                )
