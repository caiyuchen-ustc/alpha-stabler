...
def main():
    ...
    for f in files:
        ...
        for batch in pf.iter_batches(batch_size=512):
            ...
            if args.prompt_key not in df.columns or args.gen_key not in df.columns:
                raise KeyError(
                    f"{f} missing columns; has {list(df.columns)}, "
                    f"need '{args.prompt_key}' and '{args.gen_key}'."
                )
    ...
    if writer is not None:
        writer.close()
    print(
        f"Wrote {n_written} (prompt, response) pairs to {args.output} "
        f"(dropped {n_dropped} empty/too-short responses)."
    )
